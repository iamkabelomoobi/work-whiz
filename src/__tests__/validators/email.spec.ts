...
describe("emailValidator", () => {
  ...
  it("should return error for an empty string", () => {
    ...
    expect(error?.details[0].message).toBe("Email cannot be just spaces or empty.");
  });
  ...
  it("should return error for a blocked domain (protonmail.com)", () => {
    ...
    expect(error).toBeDefined();
    expect(error?.details[0].message).toBe("We currently do not accept emails from that provider.");
  });
